...
// ── Tool context metadata (for scoring bonuses) ───────────────────────────
// min/max = minutes; minP/maxP = people (null = no limit); modes = modalities
const TOOL_META = {
...
// ── Option selection (steps 1-3) ──────────────────────────────────────────
function bindOptions(containerSelector, key, next) {
    ...
    container.querySelectorAll('.pathway-option').forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            state[key] = btn.dataset.value !== undefined ? btn.dataset.value : Number(btn.dataset.value);
            if (key === 'minutes') state.minutes = Number(btn.dataset.value);
            if (key === 'people')  state.people  = Number(btn.dataset.value);
            if (key === 'modality') state.modality = btn.dataset.value;
            setTimeout(() => showStep(next), 260);
        });
    });
}

// ── Build circles (step 4) ────────────────────────────────────────────────
function buildCircles() {
    ...
    shuffled.forEach((goal, idx) => {
        ...
        // Toggle selection on circle click (but not audio button)
        div.addEventListener('click', e => {
            ...
            countBadge.textContent = state.selectedGoals.size;
            countBadge.style.display = state.selectedGoals.size ? '' : 'none';
        });
...
function computeScores() {
    ...
    for (const [slug, meta] of Object.entries(TOOL_META)) {
        ...
        if (state.minutes  && meta.min <= state.minutes)  scores[slug] += 2;
        if (state.people   && meta.minP <= state.people && (meta.maxP === null || state.people <= meta.maxP)) scores[slug] += 2;
        if (state.modality && meta.modes.includes(state.modality)) scores[slug] += 1;
    }
    return scores;
}
...
function renderResults() {
    ...
    if (top.length === 0) {
        resultCards.innerHTML = '<p class="pathway-no-results">No goals selected — please go back and choose at least one.</p>';
        return;
    }

    top.forEach(({ slug, score }, idx) => {
        ...
        card.innerHTML = `
            ...
            <div class="pathway-result-body">
                ...
                <div class="pathway-result-actions">
                    <a href="${CATALOG_URL}${slug}/draft/">Work Solo</a>
                    <a href="${CATALOG_URL}${slug}/session/start/">Start a Session</a>
                </div>
            </div>`;
...
// ── Bind option pickers ───────────────────────────────────────────────────
bindOptions('#pathway-time-options',     'minutes',  2);
bindOptions('#pathway-people-options',   'people',   3);
bindOptions('#pathway-modality-options', 'modality', 4);

// Pre-build circles on first visit to step 4 if triggered via next btn
document.getElementById('btn-to-goals').addEventListener('click', () => {
    // circles are built in the click handler above; this listener intentionally empty
}, { once: false });
